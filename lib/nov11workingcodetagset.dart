  ...
  void _onDataReceived(Uint8List data) async {
    ...
    if (!stopFlag) {
      if (manuFact) {
        ...
        print('*************************');
        print(fetchlcluclCMDData.length);
        print('*************************');
        if (fetchlcluclCMDData.length >= 35) {
          print('data fetch lclucl');
          print('*************************');
          print(fetchlcluclCMDData);
          print('*************************');
          print('sub array:${isSubArray(fetchlcluclCMDData, macaddress, 24)}');
          breaking3(
            isSubArray(fetchlcluclCMDData, macaddress, 24),
          );
        }
...
//form submittion
  ...
  void _submitForm3() async {
    ...
    if (isValidUCLLCL) {
      ...
      List<int> cmdforlclucl = Command.lcluclCmd1 +
          unitinput +
          uclinput +
          lclinput +
          Command.lcluclCmd2;
          ...
          Row(
            children: [
              ...
              manufacture.length >= 28
                  ? Flexible(
                      child: isSubArray(isSubArray(manufacture, macaddress, 10),
                                      [254], 1)
                                  .first ==
                              55
                          ? Text(
            ...
            onPressed: () async {
              ...
              await showDialog(
                ...
                builder: (ctx) => AlertDialog(
                  title: Text("Action needed"),
                  content: Text(
                      "Feild device output 20ma equal to reference meter?"),
                  actions: <Widget>[
                    ElevatedButton(
                      onPressed: () {
                        outer = true;
